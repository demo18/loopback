...
function AccessContext(context) {
  ...
  this.accessScope = context.accessScope;
...
AccessContext.prototype.isScopeAllowed = function() {
  ...
  const scopes = this.accessToken.scopes;

  if (this.accessScope === undefined) {
    // By default and for backwards compatibility reasons,
    // methods with not scope defined can be invoked only
    // by access tokens with an undefined "scopes" list
    return scopes === undefined;
  }

  // When the invoked method does provide a required scope,
  // then the access token must contain the required scope
  // in the "scopes" list.
  return Array.isArray(scopes) && scopes.indexOf(this.accessScope) !== -1;
};
...
AccessContext.prototype.debug = function() {
  if (debug.enabled) {
    ...
    debug('accessScope %s', this.accessScope || '<default>');
    if (this.accessToken) {
      debug('accessToken:');
...
AccessRequest.prototype.debug = function() {
  if (debug.enabled) {
    ...
    debug(' property %s', this.property);
    debug(' accessType %s', this.accessType);
    debug(' accessScope %s', this.accessScope);
